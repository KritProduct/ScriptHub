...
function ESP.GetTeamColor(target)
    local team = target.Team
    if not team then return ESP.Settings.Color end
            ...
            if p ~= player and p.Character then
                pcall(function()
                    ...
                    if ESP.Settings.Mode == "Team" then
                        local color = ESP.GetTeamColor(p)
                        h.FillColor = color
...
function ESP.BuildSettings(content)
    ...
    teamBtn.TextColor3 = Color3.fromRGB(180, 180, 180)
    ...
    local function updateMode()
        if ESP.Settings.Mode == "Color" then
            ...
            colorBtn.TextColor3 = Color3.fromRGB(180, 180, 180)
        end
    end
